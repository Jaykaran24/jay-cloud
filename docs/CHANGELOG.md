...
- [2026-07-07] auth.service.ts: loginUser() with mock admin bypass for dev/testing, real fetch for all other credentials; logoutUser() stub
- [2026-07-07] ProtectedRoute: HOC that redirects unauthenticated users to /login via React Router Navigate
- [2026-07-07] LoginPage: Full glassmorphism card with react-hook-form + zod, show/hide password, inline error, gradient button, dev login hint
- [2026-07-07] DashboardPage: Sprint 3 placeholder with top bar, logout, admin-only Manage Users card
...
### Changed
- [2026-07-07] auth.service.ts: Added mock credentials bypass for admin@jaycloud.local / admin1234 (no backend required for dev)
- [2026-07-07] AuthContext User type: Added name field (id, name, email, role)
- [2026-07-07] LoginPage: Added subtle dev credentials hint below the form (text-xs text-zinc-600)
...
- [2026-07-07] router.tsx: /login, /dashboard (protected), /dashboard/users (protected)
- [2026-07-07] DashboardPage: Admin-only Manage Users card
